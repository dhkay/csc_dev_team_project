/**
 * RBFR 계산 엔진 — 05_스코어링엔진.md의 직접역할 효능값/비중값 계산식.
 * `/var/www/rbfr/engine/src/scoringEngine.ts`(3단계 프로토타입)에서 이동. 계산 로직 자체는
 * 바꾸지 않았다(그룹웨어 맞춤 설계용 파일지침.md §7 "핵심 로직은 거의 그대로 이동").
 *
 * 3단계(계산 엔진) 범위: 직접역할 4개의 가산 계산식 + 2~4단계 보정계수 + 비중값 계산만.
 * 처방 검증(배합비 합계, 규제, 병용금기, HLB 등)은 RbfrValidationService가 담당한다.
 */
...
import type { RbfrScoringPort } from '../ports/inbound';
...
/** RbfrScoringPort 구현. 외부 의존성이 없어 생성자 주입이 필요 없다(순수 계산). */
@Injectable()
export class RbfrScoringService implements RbfrScoringPort {
  calculateDirectRoleEfficacy = calculateDirectRoleEfficacy;
  calculateAllDirectRoleEfficacies = calculateAllDirectRoleEfficacies;
  calculateRatioValues = calculateRatioValues;
}
