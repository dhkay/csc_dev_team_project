 ...
 * `/var/www/rbfr/engine/src/validation.ts`(4단계 프로토타입)에서 이동.
 *
 * RbfrScoringService(계산)의 산출물을 그대로 입력받아 "검증만" 한다. 계산과 검증을 같은
 * 함수에 섞지 않는다.
 *
...
import type { IngredientHlbProfile } from '../../domain/types';
 ...
 */
export function validateFormula(input: ValidateFormulaInput): FormulaValidationResult {
  const issues: ValidationIssue[] = [
...
/** RbfrValidationPort 구현. 외부 의존성이 없어 생성자 주입이 필요 없다(순수 검증). */
@Injectable()
export class RbfrValidationService implements RbfrValidationPort {
