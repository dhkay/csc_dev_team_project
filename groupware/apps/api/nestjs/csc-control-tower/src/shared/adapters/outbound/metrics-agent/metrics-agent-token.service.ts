...
/**
 * metrics-agent 호출용 서비스 토큰 발급: `X-Service-Token`(HS256)
 * service 클레임은 'csc-control-tower'(에이전트 ALLOWED_SERVICES 화이트리스트와 일치)
 * 시크릿은 전 서버 공통 SERVICE_TOKEN_SECRET. 발급 로직은 `@csc/net-utils` 위임
 */
@Injectable()
export class MetricsAgentTokenService {
  private readonly logger = new Logger(MetricsAgentTokenService.name);
