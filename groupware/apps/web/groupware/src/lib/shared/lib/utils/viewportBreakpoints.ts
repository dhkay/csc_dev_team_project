...
/**
 * 좁은 화면 ↔ 넓은 화면 경계(px). UI 모드(portrait ↔ landscape) 경계와 같다.
 *
 * 1024 = iPad Pro 12.9" 세로 폭. 비율이 아니라 가로 길이로 가르므로 세로형 피봇 모니터는
 * 세로가 길어도 이 폭을 넘어 landscape 로 분류된다.
 *
 * `app.css` 의 `--breakpoint-lg` 가 같은 값으로 못 박혀 Tailwind `lg:` 도 같은 지점에서 꺾인다.
 * 어느 쪽을 쓸지는 이렇게 가른다.
 *
 * - 클래스만 달라지면 CSS(`lg:`). 마크업이 하나로 남고 서버 렌더가 곧바로 맞는 배치를 그린다.
 * - 동작이 달라지면 스토어(`viewportModeStore`). 모달이 시트로 바뀌거나 요소를 아예 마운트하지
 *   않아야 하는 경우다.
 *
 * 같은 화면을 두 방식으로 섞어 판정하지 않는다. 한쪽만 바꾸면 경계에서 배치와 동작이 어긋나고,
 * 그 어긋남은 창을 딱 그 폭으로 맞춰야만 보인다.
 */
