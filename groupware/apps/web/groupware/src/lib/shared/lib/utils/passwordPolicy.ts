...
// user 서버 DTO 의 @Matches 규칙과 동일하게 유지한다(두 앱이 분리돼 공유 모듈을 두지 않으므로 수동 동기화)
// 대상: apps/api/nestjs/user/src/domains/user/adapters/inbound/http/dto/ 의
//   create-organization / create-org-member / create-admin / reset-root-password /
//   reset-member-password / change-password
// 규칙을 바꿀 때는 control-tower 의 같은 파일까지 모두 함께 수정한다.
