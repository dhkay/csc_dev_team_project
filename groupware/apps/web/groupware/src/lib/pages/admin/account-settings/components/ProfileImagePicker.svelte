<script lang="ts">
  // 프로필 이미지 선택: 선택 시점엔 업로드하지 않고 메모리 미리보기만 한다(고아 방지)
  // 실제 업로드/영속은 폼 저장 시 부모가 accountService.uploadProfileImage(file) 로 수행한다.
  //   value      = 제출용 값 = uploadId(불변, 표시엔 미사용). displayUrl = 기존 이미지 서명 표시 URL.
  //   file       = 새로 고른 미저장 파일(저장 시 부모가 업로드해 value=새 uploadId 로 교체)
  import { validateImageFile, IMAGE_ACCEPT } from '$lib/shared/lib/image/imageFile';
  import ProfileAvatar from '$lib/shared/ui/ProfileAvatar.svelte';

  interface Props {
    // 제출용 uploadId(바인딩). 없으면 null.
    value?: string | null;
    // 기존 이미지 표시용 서명 URL(읽기 전용). 새 파일 미선택 시 미리보기로 쓴다.
    displayUrl?: string | null;
    // 새로 선택한 미저장 파일(바인딩): 저장 시 부모가 업로드
    file?: File | null;
  }
  let { value = $bindable(null), displayUrl = null, file = $bindable(null) }: Props = $props();

  let error = $state('');
  let cleared = $state(false); // '제거' 로 기존 이미지를 지웠는지
  ...
  // 표시값: 새 파일 우선, 없으면(제거 안 했으면) 기존 서명 표시 URL.
...
<div class="flex items-center gap-3">
  <ProfileAvatar src={shown} sizeClass="h-16 w-16" iconClass="h-10 w-10" />
  <div class="space-y-1">
