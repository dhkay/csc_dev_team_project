...
<script lang="ts">
  // 커스텀 오디오 플레이어: 재생 컨트롤(재생/일시정지 + 시간)과 스크럽바(재생 위치)를 분리
  //   좁은 그리드 카드에서도 스크럽바가 폭 전체를 독립적으로 차지해 재생 위치 이동이 편하다.
  //   네이티브 <audio controls> 대체(볼륨/속도 등은 미노출: 필요 시 컨트롤 행에 가산). 재생은 한 번에 하나만(위 module)
...
<div class="flex flex-col gap-1.5">
  ...
  <div class="flex items-center gap-2">
    <button
      ...
      class="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-fg text-surface transition hover:opacity-90 focus:outline-none focus-visible:ring-2 focus-visible:ring-fg/30"
    >
      {#if playing}
    ...
    <span class="ml-auto text-[11px] tabular-nums text-fg-subtle">{fmt(current)} / {fmt(duration)}</span>
  </div>
</div>

<style>
  /* 스크럽바: 얇은 트랙 + 진행 채움(--pct). 라이트/다크 모두 대비 확보(currentColor 계열 토큰) */
  .ma-seek {
    width: 100%;
    height: 6px;
    cursor: pointer;
    ...
    background: linear-gradient(
      to right,
      var(--color-fg, currentColor) 0%,
      var(--color-fg, currentColor) var(--pct, 0%),
      var(--color-line, rgba(127, 127, 127, 0.3)) var(--pct, 0%),
      var(--color-line, rgba(127, 127, 127, 0.3)) 100%
    );
  }
  .ma-seek:disabled {
    opacity: 0.5;
