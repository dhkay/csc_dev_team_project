// 태그 값 삭제 BFF: csc-marketing `DELETE /asset-catalog/tags/:id`. 플랫폼(ai-tools-management) 전용
...
export async function DELETE(event: RequestEvent) {
  ...
  try {
    const res = await serverMarketingClient().DELETE<{ success: boolean }>(`/asset-catalog/tags/${id}`);
    return ok(res.data);
  } catch (error) {
    return mapHttpError(error, {
      fallback: '태그 삭제에 실패했습니다.',
      log: 'Delete asset tag failed',
