/**
 * 서버 모니터링 프론트 타입: csc-control-tower `/platform/servers/metrics` 응답(ServerWithStatus[])과 1:1.
 * (백엔드 계약 SSOT: apps/api/nestjs/csc-control-tower .../server-monitoring/core/domain/metrics.types.ts)
 */
...
/** 그리드 카드 1개 = 서버 메타 + 온라인 상태 + (온라인 시) 스냅샷 */
...
export type ResourceKey = 'cpu' | 'ram' | 'gpu' | 'vram' | 'disk';

// 정적 하드웨어 상세(on-demand)
