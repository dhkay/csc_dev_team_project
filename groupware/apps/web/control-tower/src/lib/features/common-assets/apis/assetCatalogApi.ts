// 태그 카탈로그(축/태그) 쓰기 데이터 접근: 같은 origin BFF(/api/platform/asset-catalog) frontClient 호출
//   현재 UI 는 추가/삭제만 사용(수정/토글 미노출) → create/delete 만 노출. 목록(GET)은 SSR.
...
const AXES = ROUTES.PLATFORM.ASSET_CATALOG_AXES;
const TAGS = ROUTES.PLATFORM.ASSET_CATALOG_TAGS;
...
export function deleteAxis(id: number): Promise<ApiResult<unknown>> {
  return run(() => frontClient().DELETE(ROUTES.PLATFORM.assetCatalogAxis(id)));
}
...
export function deleteTag(id: number): Promise<ApiResult<unknown>> {
  return run(() => frontClient().DELETE(ROUTES.PLATFORM.assetCatalogTag(id)));
}
